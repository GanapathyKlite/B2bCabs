...
const CarListNavBar: React.FC = () => {
  ...
  const { inputValueOne, inputValueTwo, selectedDate, selectedDateRange, tripType, setTripType, setInputValueOne,setInputValueTwo,
    setSelectedDate, setSelectedDateRange, hourTime, setHourTime, authToken
   } = useAuth();
  ...
  if(tripType === "Daily Rental" && startCitySuggestion && endCitySuggestion && formattedDates){
    try {
      ...
      if (response.data.status) {
            ...
            sessionStorage.setItem("canceldate", formattedDates?.start_date || "");
    ...
    if((tripType === "Cab From Airport" || tripType === "Cab To Airport") && startCitySuggestion && endCitySuggestion && formattedDate.start_date){
    ...
    if(tripType === "Hourly Rental" && startCitySuggestion && formattedDate.start_date && hourTime){
    ...
    if(tripType === "Holidays Package" && formattedDates && packageId){
      try {
       ...
       if (response.data.status) {
        ...
        setDuration("");
        setKm(0);
        setPeriod({ noOfDays: response.data.noOfDays, noOfNights: response.data.noOfNights })
    ...
    <>
      ...
      />
            ...
            <div className="arrows">
             {tripType !== "Hourly Rental" ? <LuArrowLeftRight style={{cursor: "pointer"}} onClick={swapValues}/>:null} 
            </div>
         {tripType !== "Hourly Rental" ?( <div className="p-0 col-lg-2">
              <div className="d-flex justify-content-between align-items-start fromPlace">
...
>
      ...
      <CarHero cars={carData} duration={duration} start_date ={formattedDate.start_date} endrangedate = {formattedDates?.end_date} hourTime = {hourTime}
            pickup_time={formattedDate.pickup_time} startrangedate = {formattedDates?.start_date} km={km} period={period} cancelDate ={cancelDate} errorMessage = {errorMessage}/>
    </>
  );
};
